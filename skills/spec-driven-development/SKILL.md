...
Execute tasks one at a time following `incremental-implementation` and `tdd`. Load only the spec sections and source files needed for the current task rather than flooding the agent with the entire spec.
