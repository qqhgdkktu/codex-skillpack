...
- If the user asks to build or update a full-page screen in Figma from code or a description, use `figma-use` plus the Figma MCP directly.
- If the user asks only for Code Connect mappings, treat that as out of scope for this lean bundle unless a dedicated Code Connect skill/tool is installed.
- If the user asks to author reusable agent rules, write them directly in `AGENTS.md` or the project's existing rule file.
